...
- 詳細ビューアーは前後移動、キーボードの方向キー、スワイプ操作に対応し、閲覧方向に応じて隣接画像を先読みします。
- 背景画像、ポスター、アバター、ページネーションを備えたメディアライブラリ形式の作品・キャラクターページ。
- レーティングは `safe`、`sensitive`、`hidden` の 3 種類で固定されています。
- 公開ランダム画像 API は作品、キャラクター ID、中国語名、日本語原名、別名、レーティング、画像方向で絞り込め、PC とモバイルの既定方向を個別に設定できます。
- 管理画面では表形式と Masonry 形式を切り替えられ、どちらも一括操作に対応しています。
- 管理者プロフィールではアバター、ヘッダー表示用ニックネーム、ログイン名を管理でき、アカウント設定へのショートカットも利用できます。
...
- 更新センターでは更新タスクの履歴をページ分割し、長いログは詳細パネル内でスクロールできます。
- 画像アップロード画面はアクティブなタスクを初期表示し、完了・失敗・キャンセル済みの履歴はステータスフィルターから確認できます。

## 画像処理

| アップロード形式 | 保存形式 | ブラウザでの表示 |
| --- | --- | --- |
| 一般的な静止画 | WebP に変換 | WebP のマスター画像とサムネイル |
| GIF などのアニメーション画像 | 元のアニメーション形式を保持 | 元のアニメーションと静止 WebP サムネイル |
...
通常の SDR 静止画とアニメーション画像では、恒久的なプレビュー画像を別途保持しません。ギャラリーカードにはサムネイルを使い、詳細ページではマスター画像を読み込みます。HDR 画像だけは、標準ダイナミックレンジの画面向けに SDR WebP プレビューを保持します。
