...
<p align="center">
  ...
  <a href="docs/api/operations-guide.md">API 操作說明</a> |
...
- 圖片詳情浮層及獨立詳情路由。
- 詳情檢視器支援上一張/下一張、鍵盤方向鍵與滑動瀏覽，並會依瀏覽方向預載相鄰圖片。
...
- 管理員資料支援頭像、頂部顯示暱稱、登入使用者名稱，以及直接前往帳號設定的捷徑。
...
- 更新中心會將更新任務歷史分頁顯示，長日誌會在詳情框內獨立捲動。
- 圖片上傳頁預設只顯示進行中的任務；已完成、失敗與已取消的歷史記錄仍可透過狀態篩選查看。

## 圖片管線

| 來源 | 原圖入庫 | 瀏覽器預覽 |
| --- | --- | --- |
| 一般靜態圖片 | 轉為 WebP | WebP 主圖與縮圖 |
| GIF / 動圖 | 保留動畫格式 | 原始動畫與靜態 WebP 縮圖 |
...
一般 SDR 靜態圖片與動圖不會額外長期保留預覽檔：圖片庫卡片使用縮圖，詳情頁載入主圖。只有 HDR 圖片會保留 SDR WebP 預覽圖，讓一般動態範圍螢幕能正常顯示。
