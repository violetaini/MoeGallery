...
<p align="center">
  ...
  <a href="docs/api/operations-guide.md">API 运维说明</a> |
...
- 全屏幻灯片首页，后台最多指定 48 张展示图片；未指定时会从图库随机选择 24 张。上方幻灯片和下方胶片始终共用同一图片 URL，先并发填满首屏胶片，再在首屏稳定后预加载其余图片。
- 瀑布流图片库，支持搜索、排序，以及按作品、角色和分级筛选。
- 滚动到底部后自动加载下一页，并提前预加载即将展示图片的缩略图。
- 点击图片即可在当前页面查看详情，同时保留可直接访问的独立详情地址。
- 详情查看器支持上一张/下一张、键盘方向键和滑动浏览，并会按浏览方向预加载相邻图片。
- 内置 CDN 缩略图预热：通过已配置 HTTPS 域名模拟浏览器访问，识别 ESA、EdgeOne、Cloudflare 缓存状态，支持持久重试并在共享缓存到期前自动续热。
...
- 管理员资料支持头像、顶部显示昵称、登录用户名，以及直达账号设置的快捷入口。
...
- 更新中心会分页显示更新任务历史，长日志在详情框内独立滚动。
- 图片上传页默认只显示进行中的任务；已完成、失败和已取消的历史记录仍可通过状态筛选查看。

## 图片处理

| 上传类型 | 入库存储 | 浏览器展示 |
| --- | --- | --- |
| 普通静态图片 | 转换为 WebP | 使用 WebP 主图和缩略图 |
| GIF 或其他动图 | 保留原动画格式 | 使用原始动图和静态 WebP 缩略图 |
...
普通 SDR 静态图片和动图不会额外长期保留预览文件：图片库卡片使用缩略图，详情页加载主图。只有 HDR 图片会保留 SDR WebP 预览图，以便普通动态范围屏幕正常显示。
