...
HDR JPEG XR uploads are transcoded to HDR AVIF originals through `ffmpeg` while still generating SDR WebP preview and thumbnail derivatives. SDR static images and animated images keep only a master plus thumbnail; the image detail view falls back to that master when no preview exists.
...
The current limiter stores counters in process memory, which is enough for local or single-process deployments. For multi-process or multi-instance production deployments, move the counter to shared storage such as Redis. If the app is behind a reverse proxy, make sure the proxy overwrites user-supplied forwarding headers. Only loopback proxies are trusted by default; add exact proxy networks with `AGMS_TRUSTED_PROXY_CIDRS` and never trust all addresses.
