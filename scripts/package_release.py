    ...
    (stage_root / "RELEASE_NOTES.md").write_text(
        "\n".join(
            [
                ...
                "# configure Nginx, then open /install",
                "```",
                "",
                "For a clean deployment, configure SQLite/MySQL, the admin account, and the session secret in the web installer.",
                "`install_systemd.sh` also installs the independent updater service and writes the default panel update trigger when missing.",
                "Manual `.env` editing is only needed when bypassing the installer or upgrading an existing deployment.",
                "",
                "Upgrade an existing deployment:",
                "",
                "```bash",
                "sudo bash /opt/anime-gallery/scripts/upgrade_release.sh /tmp/MoeGallery-<version>.tar.gz",
                "```",
                "",
