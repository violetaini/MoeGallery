...
Options:
  ...
  --service NAME       systemd service name. Default: anime-gallery
  --health-url URL     Health check URL. Default: http://127.0.0.1:8000/api/health
...
if [[ "$SKIP_BACKUP" -eq 0 ]]; then
  if [[ "$DRY_RUN" -eq 1 ]]; then
    ...
    "$SCRIPT_DIR/backup_before_upgrade.sh" --app-dir "$APP_DIR" --backup-root "$BACKUP_ROOT"
  fi
else
...
for file in .env.example LICENSE README.md README_zh.md README_zh-TW.md README_ja.md VERSION RELEASE_NOTES.md; do
...
if [[ -x "$APP_DIR/scripts/create_linux_dirs.sh" ]]; then
  run bash "$APP_DIR/scripts/create_linux_dirs.sh"
fi
